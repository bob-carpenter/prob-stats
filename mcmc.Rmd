...
own work, CC-BY-SA 3.0</span>] are just methods that use simulations
of probabilistic processes to calculate numerical solutions to
definite integrals, usually in high dimensions.  We have seen numerous
examples in earlier chapters, mostly involving the estimation of
expectations, including event probabilities and parameter estimates.
In all of the examples we have seen, we have been able to generate
independent simulation draws.^[The advantage is that the central limit
theorem applies as is to independent draws.]

For most statistical models of applied interest, no methods exist for
taking independent draws from the posterior.  In many cases, we can
take

The best we can do is
take a sequence of draws from the posterior that are correlated with
one another.  The most common way for doing that is by generating
the draws from a Markov chain whose elements are correlatedequilibrium distribution is that
of the target.
...
correlated (or anti-correlated) as a sequence.
...
The state diagram indicates that if the previous draw was
...
```{r}
corr_trans <- matrix(c(0.1, 0.9,
                       0.9, 0.1), 2, 2)
ind_trans <- matrix(c(0.5, 0.5,
                      0.5, 0.5), 2, 2)
anti_trans <- matrix(c(0.9, 0.1,
                       0.1, 0.9), 2, 2)
M <- 1e4
