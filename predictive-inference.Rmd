...
collection as output, then we can write this using functional
operators as
...
p(\tilde{y} \mid y)
& \approx &
\mbox{mean}(\mbox{binomial}(\tilde{y} \mid \tilde{N}, \theta)).
\\[6pt]
& = &
\mbox{mean}(\mbox{binomial}(\tilde{y} \mid \tilde{N}, \theta^{(1)}),
            \ldots,
	    \mbox{binomial}(\tilde{y} \mid \tilde{N}, \theta^{(M)}))
\\[6pt]
& = &
...
Let's continue with the small data $y = 3$ and $N = 10$.  We'll take
$\tilde{N} = 20$ and calculate the results for all possible $\tilde{y}
\in 0:\tilde{N}$, then plot them.
...
pp1_df <- data.frame(y_pred = 0:20, log_p = exp(log_pred))
pp1_plot <-
  ggplot(pp1_df, aes(y_pred, log_p)) +
...
outcomes.  Contrast this situation to using a simple binomial to do
prediction setting $\theta = 0.3$, the observed proportion of boys.
...
it is in fact an opportunity.  By rearranging terms, we have
