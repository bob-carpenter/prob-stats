...
mean, standard deviation, and quantiles being common.  These are all
easily calculated based on the sample draws.  For instance, the
posterior mean and standard deviation are as simple as calling
built-in mean and standard deviation functions
...
print 'estimated posterior central 80% interval = '
...
printf('estimated posterior median = %3.2f\n',
       quantile(theta, 0.2))
printf('estimated posterior central 80 pct interval = (%3.2f, %3.2f)\n',
       quantile(theta, 0.1), quantile(theta, 0.9))
```

This answers the question pretty thoroughly about Laplace's questions
about estimating $\theta$.  To answer the question about whether boys
are more prevalent than girls, we need to estimate $\mbox{Pr}[\theta >
0.5]$, which is straightforward with simulation.
...
they represent more than 50% of the live births is
