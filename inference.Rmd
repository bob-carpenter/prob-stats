...
from it. *Jahrbuch der Radioaktivitﬁt and Elektronik* 4:411--462.]
which itself was only a special case of the more general theory of
relativity.^[Einstein, Albert. 1916. The foundation of the general
theory of relativity. *Annalen Phys.* 14:769--822.]  Each successive
...
With a prior and sampling distribution, we have our full joint model
in hand,
...
For reference, these are the sampling distribution
$$
\mbox{binomial}(y \mid N, \theta)
\ \propto \
...
It's hard to glean much from the draws.  Instead, we'll want to look
at summaries of the posterior.  For visualizing draws of a single
variable, histograms are handy.
...
skewed, but that is not clearly visible by inspection. It's also hard
to tell the exact location of the posterior mean and median.
...
The median (aka, the 50% quantile), is just slightly lower than the
mean. This answers Laplace's question about estimating $\theta$ as a
distribution, then summarizing those estimates. To answer the question
about whether boys are more prevalent than girls, we need to estimate
$\mbox{Pr}[\theta > 0.5]$, which is straightforward with simulation.
As usual, we just count the number of times that the simulated value
$\theta^{(m)} > 0.5$ and divide by the number of simulations $M$,

```
print 'Pr[theta > 0.5] = ' sum(theta > 0.5) / M
```

Running this, we see that with 4 boys in 10 births, the probability
boys represent more than 50% of the live births is estimated, relative
to the model, to be

```{r}
printf('Pr[theta > 0.5] = %3.2f\n', sum(theta > 0.5) / M)
```

In a normal distribution, plus or minus one standard deviation covers
the quantiles $(0.159, 0.841)$, whereas plus or minus two standard
deviations covers $(0.023, 0.978)$.

Now let's overlay the median and central 95% interval (equivalently,
the two 2.5% tail intervals) onto our fine-grained histogram.

```{r fig.cap = "Histogram of $1\\,000\\,000$ draws from the posterior $$p(\\theta \\mid y, N) \\propto \\mbox{binomial}(y \\mid N, \\theta),$$ given $N = 10, y = 3$.  The median (50% quantile) is indicated with a dashed line and the boundaries of the central 95% interval (2.5% and 97.5% quantiles) are picked out with dotted lines.  The proportion of the total area shaded to the right of 0.5 represents the probability given our observed data that $\\theta > 0.5,$ which is about 11%."}

binomial_post_plot3 <-
  ggplot(binom_post_df2, aes(x = theta)) +
  geom_histogram(color = 'black', fill = '#ffffe6',
                 size = 0.25, binwidth = 0.01, boundary = 0.5) +
  geom_histogram(data = subset(binom_post_df2, theta > 0.5),
                 color = 'black', size = 0.25,
		 fill = '#ccccb6', binwidth = 0.01, boundary = 0.5) +
  geom_vline(xintercept = quantile(binom_post_df2$theta, 0.5),
             linetype = "dashed", size = 0.5) +
  geom_vline(xintercept = quantile(binom_post_df2$theta, 0.025),
             linetype = "dotted", size = 0.5) +
  geom_vline(xintercept = quantile(binom_post_df2$theta, 0.975),
             linetype = "dotted", size = 0.5) +
  scale_x_continuous(lim = c(0, 1),
                     breaks = c(0, 0.25, 0.5, 0.75, 1)) +
  xlab(expression(theta)) +
