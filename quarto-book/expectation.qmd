...
involves first calculating the expectation, it can't be done within a
single Stan program.  Here's code to calculate it on the outside.
...
A better way to do this calculation is with NumPy's `var()` function,
function,
...
```{python}
fit_die.summary(sig_figs=5, percentiles=(50,))
```

The output includes rows for two variables `lp__` and `z`.  The
variable `lp__` is not relevant when we only have a generated
quantities block, so we will ignore it for now and focus on the values
for `z`.  The header has a short description of the values, the first
...
* The second column is the Monte Carlo standard error (MCSE); when
things are working properly with sampling and we have more than a few
dozen draws, the mean will usually be within two or three standard
errors of its true value.^[Later, we will see this means there is an
approximately 95% chance the mean is within two standard errors and
99% chance it is within three.]  We want this value to be as low as
possible.  The MCSE is technically redundant as it is defined to be
the standard deviation (third column) divided by the square root of
the effective sample size (fifth column).

* The third column, headed "StdDev", is the sample standard deviation
of the draws.  Its value matches the sample standard devbiation of the
draws we computed manually above.

* The fourth column, headed "50%", is the median value, which is not
particularly relevant for discrete values; the value might have been 4
with only slightly different simulated values.

* The fifth column, headed `N_eff`, reports the effective sample
...
* The sixth column, headed n`N_eff/s`, reports `N_eff` per second.

* The seventh and final column, `R_hat`, reports a diagnostic of
