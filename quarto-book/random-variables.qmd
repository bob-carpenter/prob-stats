...
counts = Counter(z)
for n in range(2, 13):
  print("Pr[Z = {0:2d}] = {1:5.3f}".format(n, counts[n] / len(z)), end="   ")
  if ((n - 1) % 3 == 0): print("")
```
...
In the case of two dice, it's easy enough to calculate the probability
of each outcome analytically.  Because we have two six-sided dice,
there are 36 possible combinations $(d_1, d_2)$, where $d_1$ is the
result of the first die and $d_2$ the second. Each outcome $(d_1,
d_2)$ is equally probable, so the total probability of an outcome is
the number of ways it can be produced divided by the total number of
outcomes. 
