...
We read $\mbox{Pr}[\mathrm{A} \mid \mathrm{B}]$ as the *conditional probability* of
event $\mathrm{A}$ given that we know event $\mathrm{B}$ occurred.  Knowing that event
$\mathrm{B}$ occurs often gives us information about the probability of $\mathrm{A}$
occurring.
...
What is nice is that we can compute marginals directly---it is just as
easy to compute the marginal of $Y$, the test report, as it is to
compute the marginal of $Z$, the disease status.  Let's run that with
$M = 100\,000$ and see what we get.
...
the top row, $\mbox{Pr}[Z = 1, Y = 1]$ and $\mbox{Pr}[Z = 1, Y = 0]$.
...
column, $\mbox{Pr}[Z = 1, Y = 0]$ and $\mbox{Pr}[Z = 0, Y = 0]$.
...
Letting $\mathrm{A}$ be the event $Y = y$ and letting $\mathrm{B}$ be the event $Z = z$,
Bayes's rule can be expressed using probability mass functions as
...
to evaluate a formula like Fermat evaluated $\mbox{Pr}[Z = 1 | Y_{n,
