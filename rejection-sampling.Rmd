...
$F^{-1}_Y(p)$ can be computed by solving $$F_Y(u) = \int_{\infty}^u p_Y(y)
\mathrm{d} y = p$$ for $u$, which can typically be accomplished
...
1$ and $\beta = N - y + 1$. In general, if a random variable $\Theta
...
We can see that when the mean is 0.5 and the count is 2, we have
$\alpha = \beta = 1$, and the result is a uniform distribution,
...
& = & 1
\\[4pt]
& = & \mbox{uniform}(y \mid 0, 1).
\end{array}
$$

We should thus be thinking of $\alpha + \beta = 2$ as being equivalent
to a total count of zero, as a total count of 2 is required to lead to
a uniform distribution.^[Arguably, the beta distribution would be more
naturally parameterized with parameters $\alpha, \beta > -1$ to
naturally scale the "counts" and avoid subtraction in the exponent
expressions.]
...
we will create a box with height 5 and width of 1.^[In general, we
could use an optimization algorithm to find the largest value.] Next,
we draw uniformly from the box, drawing a horizontal position
$\theta^{(m)} \sim \mbox{uniform}(0, 1)$ and vertical position
$u^{(m)} \sim \mbox{uniform}(0, 5)$. The points whose value for $u$
falls below the density at the value for $\theta$ are retained.
...
with patience, another 10 dimensions becomes completely unworkable,
