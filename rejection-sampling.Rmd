...
## Beta distribution
As a first non-trivial example, let's consider the
beta distribution.^[We'll have a lot more to say about beta
distributions as we go along as they have a close connection to binary
trial data.]  If $\Theta$ has a beta distribution, then its density
$p_{\Theta}$ is
...
For example, here is a plot of a few beta distributions.
...
concentrate most of the probability mass near the boundaries. For many
of these distributions, there is no mode as the density grows without
limit as it approaches zero and/or one. As the count grows, the
probability mass concentrates away from the boundaries and around the
mean.
...
case of the beta distribution, draws are bounded between 0 and 1.
And lets's start with a $\mbox{beta}(6.8, 1.2)$ distribution,
corresponding to the mean 0.675 and count of 8 case from the previous
section.
...
More specifically, we keep the points $\theta^{(m)}$ where
...
sample $(u, \theta)$ uniformly until we find one meeting the above
condition and return the $\theta$ value.
