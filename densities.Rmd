...
The trick is to make the sorted random variable the $x$-axis in the
plot.  Here's a run with $M = 1\,000$ simulated values.
...
Let's check this with some simulation.  We know how to simulate
logistic random variables---let's estimate their expectation and
variance.  We just perform draws then calculate means and variances,

```
for (m in 1:M)
  y[m] = logit(uniform_rng(0, 1))
E_Y = sum(y) / M
var_Y = sum((y - E_y)^2) / M
print 'Estimated E[Y] = ' E_Y
      '; var[Y] = ' var_Y
      '; sd[Y] = ' sqrt(var_Y)
```
...
E_Y <- sum(y) / M
var_Y <- sum((y - E_Y)^2) / M
printf('Estimated  E[Y] = %3.2f;  var[Y] = %3.2f;  sd[Y] = %3.2f',
       E_Y, var_Y, sqrt(var_Y))
```
