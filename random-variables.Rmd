...
```{r fig.cap="The log of the absolute error. With both axes on the log scale, the slope of the relationship between the number of draws and estimation error is revealed. The blue line is drawn at the 95% quantile of estimates and one at the 68% quantile."}
...
log_abs_err_plot <- ggplot(df2, aes(x = M, y = err_hat_E_Y + fudge, group=r)) +
  geom_line(alpha=0.15) +
  geom_point(data = data.frame(Ms = Ms, sd = sd),
            aes(x = Ms, y = sd),
  ...
  geom_line(data = data.frame(Ms = Ms, sd = sd),
            aes(x = Ms, y = sd),
            color = "blue", group = NA) +
  geom_line(data = data.frame(Ms = Ms, sd2 = 2 * sd),
            aes(x = Ms, y = sd2),
            color = "red", group = NA) +
  scale_x_log10(limits=c(.9 * 1e4, 1e6 * 1.1),
                breaks=10^(4:6),
                labels=c("10,000", "100,000", "1,000,000")) +
