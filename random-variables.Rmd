...
of these two lines is $-\frac{1}{2}$.  If we let $\epsilon$ be the
absolute error and $M$ the number of simulation draws, we see the
linear relationhip 

$$
\log \epsilon = -1/2 * (\log M) + \mbox{const}
$$

Going back to the linear scale requires exponentiating both sides of
...
\exp(\log \epsilon)
& = &
\exp( -\frac{1}{2} \times \log M + \mbox{const} )
\\[6pt]
%
\epsilon
& = &
\exp( -\frac{1}{2} \times \log M )
\times
\exp(\mbox{const})
%
\\[6pt]
...
\epsilon
& = &
\mbox{const} \times M^{-\frac{1}{2}}
\end{array}
$$

Dropping the constant, this relationship between the expected absolute
error $\epsilon$ and the number of simulation draws $M$ may be
...
This is the main component of the *central limit theorem*, which
governs the accuracy of almost all simulation-based estimates.
