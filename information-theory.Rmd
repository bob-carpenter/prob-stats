...
\mathbb{E}[- \log p_Y(Y)].
$$
Because we are using the natural logarithm, the units of entropy are
called *nats*.^[Information theory is often expressed using base 2
logarithms, which yields units of bits, which are easily interpretable
as the amount of information in a coin flip, i.e., if 
$Y \sim \\textrm{bernoulli}(\\frac{1}{2}).$, entropy in bits is 1.]
...
& = & \mathbb{E}[\log p_Y(y)]
\\[4pt]
& = & \sum_{y \in \mathbb{Z}} p_Y(y) \cdot \log p_Y(y).
\end{array}
$$
...
$$
We can plot what this looks like for $\theta \in (0, 1)$ to get a
sense for the behavior of entropy in a simple case.

```{r, out.width = "100%", fig.asp = 0.4, fig.cap = "Left) entropy of a variable $Y \\sim \\textrm{bernoulli}(\\theta)$ as a function of $\\theta.$ Right) standard deviation of the same variable as a function of $\\theta.$ The shapes are not identical, but share the same mode of $\\theta = 0.5$ and mimima at $\\theta = 0$ and $\\theta = 1.$"}
...
We can think of entropy as being how much information knowing a
variable's distribution provides about it.  The divergence between two
distributions is how much less information knowing a related
distribution provides about a variable.  Like entropy, divergence only
depends on the distributions and is usually expressed directly as a
function on distributions.

Suppose $p(y)$ and $q(y)$ are two probability functions.  If $p$ and
$q$ are discrete, then the divergence from $p$ to $q$ is defined by
$$
\begin{array}{rcl}
...
\end{array}
$$
this illustrates that the KL-divergence measures how much worse a
probability function $q(y)$ is than a variable's own probability
function $p_Y$ when it comes to expected negative log probability of
$Y.$  Breaking this down, $-\mathbb{E}[\log q(Y)]$ is the expected
negative log density of $Y$ under probability function $q(y)$, whereas
$\textrm{H}[Y]$ is the expected negative log density of $Y$ under its
own probability function $p_Y(y).$
